...
export async function generateStarRating(
  baseUrl: string, 
  code: string, 
  size: RatingSize,
  labels?: Labels
  ...
  try {
    ...
    const stars = Array.from({ length: 5 }, (_, i) => i + 1).map(value => {
      const url = new URL(`rate/${code}`, baseUrl);
      url.searchParams.set('type', 'stars');
        ...
        <td style="padding:0 ${sizeConfig.spacing} 0 0; text-align:center; vertical-align:middle;">
          <a href="${url.toString()}" style="text-decoration:none; display:inline-block;">
            <img 
              src="${iconUrl}" 
              alt="${value} stars" 
              title="${value} ${translations.stars}"
    ...
    console.error('Failed to generate star rating:', error);
    return '';
  }
}